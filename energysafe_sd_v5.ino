 ...
 *  - Credenciais Wi-Fi/API movidas para config.h (excluído do git)
 ...
 *
 *  [BLUETOOTH BLE UART — para testes sem conversor USB-Serial]
 *  - NimBLE UART em vez de BluetoothSerial (economia: ~420KB de flash)
 *  - Perfil Nordic UART Service (NUS) — compatível com nRF Toolbox,
 *    Serial Bluetooth Terminal (modo BLE) e apps similares
 *  - Nome BT configurável (BT_DEVICE_NAME)
 *  - Toda saída de Serial é espelhada via BLE quando cliente conectado
 *  - Comandos interativos via BLE:
 *      'r' → força leitura/publicação imediata
 *      's' → imprime stats atuais
 *      'w' → imprime status do Wi-Fi
 *      'd' → imprime dados do último snapshot de cada canal
 *      'e' → reinicia ESP32 (soft reset)
 *      'f' → imprime número de linhas pendentes no SD
 *
 *  ARQUITETURA:
 *  ┌────────────────────────────────────────────┐
...
// ── Inclua suas credenciais em config.h (não versionar no git!) ──
// Crie o arquivo config.h com o conteúdo abaixo e adicione ao .gitignore:
//
//   #pragma once
//   #define WIFI_SSID_SECRET     "seu_ssid"
//   #define WIFI_PASS_SECRET     "sua_senha"
//   #define API_URL_SECRET       "https://backendsafe.onrender.com/medicoes/"
//
// Se não quiser usar config.h, defina os valores diretamente abaixo
// na seção CONFIGURAÇÕES e remova o #include.
//
#if __has_include("config.h")
  #include "config.h"
#else
  // Fallback direto (menos seguro — evite commitar com valores reais)
  #define WIFI_SSID_SECRET     "SEU_SSID_AQUI"
  #define WIFI_PASS_SECRET     "SUA_SENHA_AQUI"
  #define API_URL_SECRET       "https://backendsafe.onrender.com/medicoes/"
#endif
...
// ─── Bluetooth BLE UART ─────────────────────────────────────
// Para desabilitar e economizar ~150KB de flash, defina como 0.
#define BT_SERIAL_ENABLE 1
  ...
  #include <BLEUtils.h>
  #include <BLE2902.h>
  #define BT_DEVICE_NAME   "EnergySafe-ESP32"
  #define NUS_SERVICE_UUID "6E400001-B5A3-F393-E0A9-E50E24DCCA9E"
  #define NUS_RX_UUID      "6E400002-B5A3-F393-E0A9-E50E24DCCA9E"
  #define NUS_TX_UUID      "6E400003-B5A3-F393-E0A9-E50E24DCCA9E"
#endif

// Macros de log — definidas aqui pois são usadas em todo o arquivo
#if BT_SERIAL_ENABLE
  #define LOGBT(msg)       do { Serial.print(msg);   bleSend(msg); bleSend("\n"); } while(0)
  #define LOGBTln(msg)     do { Serial.println(msg); bleSend(msg); bleSend("\n"); } while(0)
  #define LOGBTf(fmt, ...) do { Serial.printf(fmt, ##__VA_ARGS__); blePrintf(fmt, ##__VA_ARGS__); } while(0)
#else
...
// ─── Wi-Fi ─────────────────────────────────────────────────
// [v5.0] Credenciais lidas de config.h — não hardcoded aqui
...
const int PINO_TENSAO[3]   = {33, 25, 26};

// ─── Calibração do SCT-013-000 (100A:50mA) ─────────────────
const float CT_RATIO     = 2000.0f;
const float BURDEN_OHMS  = 33.0f;
const float CAL_GAIN_I   = 1.0f;

// ─── Calibração do ZMPT101B ────────────────────────────────
const float CAL_GAIN_V   = 234.26f;
...
// ─── Intervalos ────────────────────────────────────────────
#define PUBLISH_INTERVAL_MS   600000UL   // 10 minutos
...
// Snapshot do último ciclo para exibição via BT
...
// ═══════════════════════════════════════════════════════════
//  IMPLEMENTAÇÃO BLE UART — aqui após structs e variáveis
//  (necessário pois usa String do Arduino e tipos locais)
// ═══════════════════════════════════════════════════════════
#if BT_SERIAL_ENABLE

void processarComandoBT(char cmd); // forward declaration (impl. no final)
...
class BLERxCB : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* c) override {
    ...
    if (val.length() > 0) processarComandoBT(val[0]);
  }
};

void bleSend(const char* msg) {
  if (!bleConnected || !pTxChar) return;
  pTxChar->setValue(String(msg));
  pTxChar->notify();
}
...
void initBLE() {
  ...
  pSvc->start();
  BLEDevice::startAdvertising();
  Serial.printf("[BLE] Iniciado: %s | Nordic UART Service\n", BT_DEVICE_NAME);
  Serial.println("[BLE] App: nRF Toolbox ou Serial BT Terminal (modo BLE)");
  Serial.println("[BLE] Cmds: r=pub s=stats w=wifi d=dados e=reset f=SD ?=help");
}

#endif // BT_SERIAL_ENABLE
...
void setup() {
  ...
  LOGBTln("  Segurança reforçada | Bluetooth Serial");
  printSeparador();
  ...
  // [v5.0] Bluetooth BLE UART (NimBLE — substitui BluetoothSerial)
#if BT_SERIAL_ENABLE
  initBLE();
#endif
...
void loop() {
  ...
  if (now - lastPublish >= PUBLISH_INTERVAL_MS) {
    ...
    if (xSemaphoreTake(xResultMutex, pdMS_TO_TICKS(500)) == pdTRUE) {
      ...
      memcpy(ultimoSnap, resultados, sizeof(resultados)); // salva para BT 'd'
  ...
  // ── 4. [v5.0] Comandos via BLE UART ─────────────────────
  // BLE usa callbacks (BLERxCB::onWrite) — sem polling necessário.
  // Comandos são processados automaticamente ao receber dados.
...
void processarCanal(int canal, float* bufV, float* bufI, int n,
  ...
  for (int k = 0; k < n; k++) {
    float v = bufV[k] * CAL_GAIN_V;
    float i = bufI[k] / BURDEN_OHMS * CT_RATIO * CAL_GAIN_I;
    sumV2 += (double)v * v;
    sumI2 += (double)i * i;
    sumVI += (double)v * i;
  }
...
String medicaoParaJSON(const Medicao& m) {
  ...
  doc["tensao_rms"]        = serialized(String(m.tensao_rms,       2));
  doc["corrente_rms"]      = serialized(String(m.corrente_rms,     3));
  doc["potencia_ativa"]    = serialized(String(m.potencia_ativa,   1));
  doc["potencia_aparente"] = serialized(String(m.potencia_aparente,1));
  doc["fator_potencia"]    = serialized(String(m.fator_potencia,   3));
  doc["frequencia"]        = serialized(String(m.frequencia,       1));
  doc["energia_kwh"]       = serialized(String(m.energia_kwh,      4));
  doc["valido"]            = m.valido;
  String json;
...
// ═══════════════════════════════════════════════════════════
//  [v5.0] COMANDOS BLUETOOTH SERIAL
//
//  Permite diagnóstico e controle remoto via BT sem precisar
//  de conversor USB-Serial físico. Muito útil para testes em
//  campo onde o computador não está disponível.
//
//  Comandos (envie o caractere pelo app BT, ex: Serial Bluetooth Terminal):
//    'r' → força publicação imediata dos dados atuais
//    's' → exibe estatísticas de envio
//    'w' → exibe status do Wi-Fi (IP, RSSI)
//    'd' → exibe último snapshot de cada canal
//    'e' → soft reset do ESP32
//    'f' → exibe número de linhas pendentes no SD
...
void processarComandoBT(char cmd) {
  switch (cmd) {
    case 'r':
      LOGBTln("[BT-CMD] Forcando publicacao imediata...");
      lastPublish = 0; // força o if no loop a disparar
      break;
    ...
    case 'e':
      LOGBTln("[BT-CMD] Reiniciando ESP32...");
      delay(200);
      ESP.restart();
      break;

    case 'f':
      LOGBTf("[BT-CMD] Linhas pendentes no SD: %d\n", contarLinhasSD());
      break;

    case '?':
    case 'h':
      LOGBTln("[BT-CMD] Comandos disponiveis:");
      LOGBTln("  r = publicar agora");
      LOGBTln("  s = estatisticas");
      LOGBTln("  w = status Wi-Fi");
      LOGBTln("  d = dados do ultimo snapshot");
      LOGBTln("  e = reiniciar ESP32");
      LOGBTln("  f = linhas pendentes SD");
      LOGBTln("  ? = esta ajuda");
      break;

    case '\r':
    case '\n':
      break; // ignora quebras de linha

    default:
      LOGBTf("[BT-CMD] Comando desconhecido: '%c'. Use '?' para ajuda.\n", cmd);
      break;
  }
}

void printStatsBT() {
  LOGBTln("===== ESTATISTICAS =====");
  LOGBTf("Uptime:        %lus\n", millis() / 1000);
  LOGBTf("Ciclos envio:  %d\n",   ciclosEnvio);
  LOGBTf("Sucessos:      %d\n",   enviosSucesso);
  LOGBTf("Falhas:        %d\n",   enviosFalha);
  LOGBTf("Gravados SD:   %d\n",   gravadosSD);
  LOGBTf("SD disponivel: %s\n",   sdAvailable ? "sim" : "nao");
  LOGBTf("Timestamp:     %s\n",   getTimestamp().c_str());
  LOGBTln("========================");
}
...
void printWifiBT() {
  LOGBTln("===== WI-FI =====");
  if (WiFi.status() == WL_CONNECTED) {
    // [v5.0] Não imprime SSID por segurança
    LOGBTf("Status: Conectado\n");
    LOGBTf("IP:     %s\n",  WiFi.localIP().toString().c_str());
    LOGBTf("RSSI:   %d dBm\n", WiFi.RSSI());
  } else {
    LOGBTln("Status: Desconectado");
    LOGBTf("Proximo backoff: %lums\n", wifiBackoff);
  }
  LOGBTln("=================");
}

void printSnapshotBT() {
  LOGBTln("===== ULTIMO SNAPSHOT =====");
  for (int i = 0; i < NUM_CANAIS; i++) {
    LOGBTf("Canal %d:\n", CANAL_IDS[i]);
    LOGBTf("  V=%.2fV  I=%.3fA\n",
           ultimoSnap[i].tensao_rms, ultimoSnap[i].corrente_rms);
    LOGBTf("  P=%.1fW  S=%.1fVA  FP=%.3f\n",
           ultimoSnap[i].potencia_ativa,
           ultimoSnap[i].potencia_aparente,
           ultimoSnap[i].fator_potencia);
    LOGBTf("  F=%.1fHz  E=%.4fkWh\n",
           ultimoSnap[i].frequencia,
           ultimoSnap[i].energia_kwh);
    LOGBTf("  Valido: %s\n", ultimoSnap[i].valido ? "sim" : "nao");
  }
  LOGBTln("===========================");
}
#endif // BT_SERIAL_ENABLE
